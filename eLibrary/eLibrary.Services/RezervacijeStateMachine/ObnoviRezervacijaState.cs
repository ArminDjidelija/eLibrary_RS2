...
{
    public class ObnovljenaRezervacijaState : BaseRezervacijeState
    {

        public ObnovljenaRezervacijaState(ELibraryContext context, IMapper mapper, IServiceProvider service) 
            : base(context, mapper, service)
        {
        }

        public override async Task<Model.RezervacijeDTOs.Rezervacije> Odobri(Database.Rezervacije request)
        {
            request.Odobreno = true;
            request.State = "Odobrena";
